    ...
    (function countElements(element) {
        for (const child of element.childNodes) {
            switch (child.nodeType) {
                case 1:
                    data.tags[child.tagName] = (!data.tags[child.tagName]) ? 1 : ++data.tags[child.tagName];

                    for (const classItem of child.classList) {
                        data.classes[classItem] = (!data.classes[classItem]) ? 1 : ++data.classes[classItem];
                    }
...
function observeChildNodes(where, fn) {
    ...
    function moCallback(mutations) {
        mutations.forEach(mutation => {
            if (mutation.addedNodes.length) {
                data.type = 'insert';
                data.nodes = Array.from(mutation.addedNodes);
            } else if (mutation.removedNodes.length) {
                data.type = 'remove';
                data.nodes = Array.from(mutation.removedNodes);
            }
    ...
    const mObserver = new MutationObserver(moCallback);
    const options = {
        'childList': true,
        'subtree': true
    };

    mObserver.observe(where, options);
}
